...
void DoubleBinaryTreeAllReduce::run(EventType event, CallData* data) {
    if (state == State::Begin && type == BinaryTree::Type::Leaf) {  // leaf.1
        ...
        stream->owner->front_end_sim_send(0, Sys::dummy_data, data_size, UINT8, parent, stream->stream_id, &snd_req,
                                          &Sys::handleEvent, nullptr);
        // receiving
        sim_request rcv_req;
        rcv_req.vnet = this->stream->current_queue_id;
        RecvPacketEventHandlerData* ehd = new RecvPacketEventHandlerData(
            stream, stream->owner->id, EventType::PacketReceived, stream->current_queue_id, stream->stream_id);
        stream->owner->front_end_sim_recv(0, Sys::dummy_data, data_size, UINT8, parent, stream->stream_id, &rcv_req,
                                          &Sys::handleEvent, ehd);
        ...
        RecvPacketEventHandlerData* ehd = new RecvPacketEventHandlerData(
            stream, stream->owner->id, EventType::PacketReceived, stream->current_queue_id, stream->stream_id);
        stream->owner->front_end_sim_recv(0, Sys::dummy_data, data_size, UINT8, left_child, stream->stream_id, &rcv_req,
                                          &Sys::handleEvent, ehd);
        ...
        RecvPacketEventHandlerData* ehd2 = new RecvPacketEventHandlerData(
            stream, stream->owner->id, EventType::PacketReceived, stream->current_queue_id, stream->stream_id);
        stream->owner->front_end_sim_recv(0, Sys::dummy_data, data_size, UINT8, right_child, stream->stream_id,
                                          &rcv_req2, &Sys::handleEvent, ehd2);
        ...
        snd_req.reqType = UINT8;
        snd_req.vnet = this->stream->current_queue_id;
        stream->owner->front_end_sim_send(0, Sys::dummy_data, data_size, UINT8, left_child, stream->stream_id, &snd_req,
                                          &Sys::handleEvent, nullptr);
        ...
        stream->owner->front_end_sim_send(0, Sys::dummy_data, data_size, UINT8, right_child, stream->stream_id,
                                          &snd_req2, &Sys::handleEvent, nullptr);
        exit();
        return;
        ...
        RecvPacketEventHandlerData* ehd = new RecvPacketEventHandlerData(
            stream, stream->owner->id, EventType::PacketReceived, stream->current_queue_id, stream->stream_id);
        stream->owner->front_end_sim_recv(0, Sys::dummy_data, data_size, UINT8, only_child_id, stream->stream_id,
        ...
        snd_req.reqType = UINT8;
        snd_req.vnet = this->stream->current_queue_id;
        stream->owner->front_end_sim_send(0, Sys::dummy_data, data_size, UINT8, only_child_id, stream->stream_id,
