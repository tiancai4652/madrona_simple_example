...
QueueLevelHandler::QueueLevelHandler(int level, int start, int end, AstraNetworkAPI::BackendType backend) {
    ...
    std::cout << "Created QueueLevelHandler for level " << level 
              << " with range [" << start << ", " << end << "]" << std::endl;
#endif
}
...
std::pair<int, RingTopology::Direction> QueueLevelHandler::get_next_queue_id() {
    ...
    if (queues.size() == 0) {
#ifndef __CUDA_ARCH__
        std::cout << "No queues available at level " << level << std::endl;
#endif
    ...
    std::cout << "Getting next queue at level " << level << ": queue=" 
              << tmp << ", direction=" << static_cast<int>(dir) << std::endl;
#endif
...
std::pair<int, RingTopology::Direction> QueueLevelHandler::get_next_queue_id_first() {
    RingTopology::Direction dir = RingTopology::Direction::Clockwise;
    if (queues.size() == 0) {
#ifndef __CUDA_ARCH__
        std::cout << "No queues available at level " << level << " for first allocation" << std::endl;
#endif
        return std::make_pair(-1, dir);
    }
...
std::pair<int, RingTopology::Direction> QueueLevelHandler::get_next_queue_id_last() {
    RingTopology::Direction dir = RingTopology::Direction::Anticlockwise;
    if (queues.size() == 0) {
#ifndef __CUDA_ARCH__
        std::cout << "No queues available at level " << level << " for last allocation" << std::endl;
#endif
        return std::make_pair(-1, dir);
    }
