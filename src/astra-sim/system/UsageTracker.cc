...
CUDA_HOST_DEVICE
custom::FixedList<std::pair<uint64_t, double>, 1024> UsageTracker::report_percentage(uint64_t cycles) {
    decrease_usage();
    increase_usage();
    
    Tick total_activity_possible = (this->levels - 1) * cycles;
    auto usage_pointer = this->usage.begin();
    Tick current_activity = 0;
    Tick period_start = 0;
    Tick period_end = cycles;
    
    custom::FixedList<std::pair<uint64_t, double>, 1024> result;
