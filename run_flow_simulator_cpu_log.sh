...
# CPU 版：开启系统层(SYS_LOG) + 流仿真器网络层(NET_LOG) 双重日志。
# 日志量大，默认写入 run_log（见下方配置）。
...
# 日志开关（系统层：SIMPLE_LOG_MODE 与 SYS_LOG 二选一）
SIMPLE_LOG_MODE=0   # 1 = 简单日志（需 SYS_LOG=0）
SYS_LOG=1           # 1 = 详细系统层日志（需 SIMPLE_LOG_MODE=0）
SYS_LOG_SPECIAL=0
SYS_LOG_TARGET_NODE=0   # 只打印该 NPU 的系统层日志（-1 或非节点号=打印全部）
...
# 运行日志输出文件（留空则直接打印到终端）
run_log=/app/report/0812-gpu编译/cpu_full_log.log
